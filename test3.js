const url = "https://qgpmafxoaamhhcvnyopy.supabase.co/rest/v1/reservations?select=id,patient_phone,pin,created_at&order=created_at.desc&limit=10";
const key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFncG1hZnhvYWFtaGhjdm55b3B5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk0NDgwMDcsImV4cCI6MjA5NTAyNDAwN30.4JwWiufHkyfxF90tTY3zo2OdJb3VsdAaNEi_Mju9bb0";

fetch(url, {
  headers: {
    apikey: key,
    Authorization: "Bearer " + key,
  }
}).then(r => r.json()).then(data => {
  console.log("Recent Reservations:");
  data.forEach(d => {
    console.log(`ID: ${d.id}, phone: ${d.patient_phone}, pin: ${d.pin}, created: ${d.created_at}`);
  });
}).catch(console.error);
